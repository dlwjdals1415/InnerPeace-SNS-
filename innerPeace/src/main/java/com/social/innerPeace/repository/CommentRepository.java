...
public interface CommentRepository extends JpaRepository<Comment,Long> {
    ...
    @Query("SELECT c FROM Comment c WHERE c.post.postNo = :postNo AND c.commentNo < :commentNo ORDER BY c.commentNo DESC")
    Page<Comment> findByPostNoAndCommentNoLessThanOrderByCommentNoDesc(@Param("postNo") Long postNo, @Param("commentNo") Long commentNo, Pageable pageable);
}
